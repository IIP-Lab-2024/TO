...
def test():
    # load model
    weight = torch.load('./model/simply_model/simply_CBAUnet_best_64_128_SSIM.ckpt')
    ...
    with torch.no_grad():
        ...
        print("Train IOU:", iouacc / len(train_loader))
        ...
        print("IOU>0.9:", ioucount2)
    print()
    weight = torch.load('./model/random_model/random_CBAUnet_best_128_64_MSE.ckpt')
    model.load_state_dict(weight)
    print("------test start best------")
    print()
    model.eval()
    with torch.no_grad():
        ...
        print("IOU>0.9:", ioucount2)
    print()
    weight = torch.load('./model/random_model/random_CBAUnet_best_128_64_SSIM.ckpt')
    model.load_state_dict(weight)
    print("------test start best------")
    print()
    model.eval()
