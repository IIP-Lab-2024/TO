...
def training():
    ...
    for epoch in range(num_epochs):
        ...
        # 保存模型的检查点
        torch.save(model.state_dict(), './model/continuous_model/continuous_CBAUnet_128_64_SSIM.ckpt')
        ...
        if val_acc >= best_acc:
            best_epoch = epoch
            best_acc = val_acc
            torch.save(model.state_dict(), './model/continuous_model/continuous_CBAUnet_best_128_64_SSIM.ckpt')
    ...
    plt.savefig('./loss/continuous_CBAUnet_128_64_SSIMLoss.png')
    plt.close()
...
def test():
    ...
    weight = torch.load('./model/continuous_model/continuous_CBAUnet_128_64_SSIM.ckpt')
...
def test1():
    # load model
    weight = torch.load('./model/continuous_model/continuous_CBAUnet_best_128_64_SSIM.ckpt')
    model.load_state_dict(weight)
