...
class encoder1(nn.Module):
    def __init__(self):
        super().__init__()
        model=preUNet()
        weight = torch.load('./model/random_model/random_best_64_128.ckpt')
        model.load_state_dict(weight)
        self.network = model
        #self.network.features[0] = nn.Conv2d(1, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        self.inc=self.network.inc
        self.x1 = self.network.down1
        self.x2 = self.network.down2
        self.x3 = self.network.down3
        self.x4 = self.network.down4
    ...
    def forward(self, x):
        ...
        # x5 = self.network.up_1(x5)
        # x5 = self.network.down_4(x4)
...
class decoder1(nn.Module):
    def __init__(self):
        ...
        model.load_state_dict(weight)
        self.network = model
        self.up1 = self.network.up1
        self.up2 = self.network.up2
        self.up3 = self.network.up3
        self.up4 = self.network.up4
        # self.c1 = self.network.right_conv_1
        # self.c2 = self.network.right_conv_2
        # self.c3 = self.network.right_conv_3
        # self.c4 = self.network.right_conv_4
        # self.out = self.network.output
