...
with np.load('data/simply_supported_beam_input_data.npz') as data:
    input_data = data['input']
with np.load('data/simply_supported_beam_target_data.npz') as data:
...
def training():
    ...
    for epoch in range(num_epochs):
        ...
        # 保存模型的检查点
        torch.save(model.state_dict(), './model/simply_model/simply_CBAUnet_64_32_SSIM.ckpt')
        ...
        if val_acc >= best_acc:
            best_epoch = epoch
            best_acc = val_acc
            torch.save(model.state_dict(), './model/simply_model/simply_CBAUnet_best_64_32_SSIM.ckpt')
    ...
    plt.savefig('./loss/simply_CBAUnet_64_32_SSIMLoss.png')
    plt.close()
...
def test():
    ...
    weight = torch.load('./model/simply_model/simply_CBAUnet_64_32_SSIM.ckpt')
...
def test1():
    # load model
    weight = torch.load('./model/simply_model/simply_CBAUnet_best_64_32_SSIM.ckpt')
    model.load_state_dict(weight)
