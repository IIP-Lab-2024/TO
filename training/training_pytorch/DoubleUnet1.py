...
class encoder1(nn.Module):
    def __init__(self):
        super().__init__()
        model=UNet()
        weight = torch.load('./model/random_Unet_best.ckpt')
        model.load_state_dict(weight)
        self.network = model
        #self.network.features[0] = nn.Conv2d(1, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        self.x1 = self.network.left_conv_1
        self.x2 = self.network.left_conv_2
        self.x3 = self.network.left_conv_3
        self.x4 = self.network.left_conv_4
        self.x5 = self.network.center_conv

    def forward(self, x):
        x0 = x
        #print(self.network)
        x1 = self.x1(x0)
       # x1 =self.network.down_1(x1)
        x2 = self.x2(x1)
        x2 = self.network.down_1(x2)
        x3 = self.x3(x2)
        x3 = self.network.down_2(x3)
        x4 = self.x4(x3)
        x4 = self.network.down_3(x4)
       # x5 = self.x5(x4)
